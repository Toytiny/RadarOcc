    ...
    def simple_test(self, img_metas, img=None, points=None, rescale=False, points_occ=None, 
        ...
        if output['output_voxels_fine'] is not None:
            if output['output_coords_fine'] is not None:
                ...
                pred_c = self.empty_idx * torch.ones_like(gt_occ)[:, None].repeat(1, fine_pred.shape[1], 1, 1, 1).float()
                pred_c[:, :, fine_coord[0], fine_coord[1], fine_coord[2]] = fine_pred.permute(1, 0)[None]
            else:
                pred_c = output['output_voxels_fine'][0]
        target_voxels = gt_occ
