...
class RadarPolarCube(BEVDepth):
    ...
    def extract_pts_feat(self, rdr_cube):
        if self.record_time:
            torch.cuda.synchronize()
            t0 = time.time()
        B, range_dim, azimuth_dim, elevation_dim = rdr_cube.size()
        
        dtype = torch.float32
        # rdr_cube = torch.mean(rdr_cube, dim=1)
        
        # pad_azi = (1, 2)  # Adding 1 pad to the left and 2 pads to the right of the azimuth dimension
        # pad_ele = (74, 75)  # Adding 74 pads to one side and 75 to the other of the elevation dimension

        # rdr_cube_padded = F.pad(rdr_cube, pad= pad_ele + pad_azi, mode='constant', value=1).to(dtype)
        # quantile_rate = 0
        ...
        for batch_idx in range(B):
            ...
            power_val = normalized_cube[range_ind, elevation_ind, azimuth_ind].unsqueeze(-1)
            # thr_vale = arr_cube.quantile(quantile_rate)
            # batch_arr = rdr_cube_padded[batch_idx]
            # range_ind, azimuth_ind, elevation_ind = torch.where(batch_arr > thr_vale)
            # power_val = batch_arr[range_ind, azimuth_ind, elevation_ind].unsqueeze(-1)
            ...
            azimuth_ind += 74
            elevation_ind += 1 #for SparsePolarRaDAREnc2x
            #SparseLiDAREnc8x
            # azimuth_ind = azimuth_ind * 2 + (1024 - 107 * 2)//2
            # range_ind += 256
