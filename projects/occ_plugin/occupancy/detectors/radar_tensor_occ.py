...
class tensorOcc(BEVDepth):
    ...
    def extract_pts_feat(self, power_val):
        if self.record_time:
            torch.cuda.synchronize()
            t0 = time.time()

        radar_data = power_val
        print(radar_data)
        
        B, d_dim, r_dim, e_dim, a_dim = radar_data.size() #1,20,256,256
        pts_enc_feats = self.pts_middle_encoder(radar_data)
    ...
    def extract_feat(self, power_val):
        """Extract features from images and points."""
        img_voxel_feats = None
        pts_voxel_feats, pts_feats = None, None
        depth, img_feats = None, None

        pts_voxel_feats, pts_feats = self.extract_pts_feat(power_val)
    ...
    def forward_train(self,
        ...
        rdr_tensor = kwargs['rdr_cube']
        voxel_feats, img_feats, pts_feats, depth = self.extract_feat(rdr_tensor=rdr_tensor)
    ...
    def simple_test(self, img_metas, img=None, points=None, rescale=False, points_occ=None, 
        ...
        voxel_feats, img_feats, pts_feats, depth = self.extract_feat(power_val=power_val)
