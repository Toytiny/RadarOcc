...
class RadarDopplerAttnEASelfSS(BEVDepth):
    def __init__(self, 
            loss_cfg=None,
    ...
    def extract_pts_feat(self, rdr_cube):
        ...
        batch_size = B = power_values.shape[0]
        list_sparse_rdr_cubes = []
        list_sp_indices = []
        for batch_idx in range(B):
            
            # Flattening the results if necessary
            final_range_inds = range_indices[batch_idx].flatten()[:43750]
            final_elevation_inds = elevation_indices[batch_idx].flatten()[:43750]
            final_azimuth_inds = azimuth_indices[batch_idx].flatten()[:43750]
            #37,107,256
            power_val = rdr_cube['power_val'][batch_idx][:,:43750]
            sparse_rdr_cube =  torch.swapaxes(power_val,0,1)
