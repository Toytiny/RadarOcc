...
class RadarOccAttn(BEVDepth):
    ...
    def extract_pts_feat(self, rdr_cube):
        ...
        radar_data = rdr_cube
        B, z_dim, y_dim, x_dim = radar_data.size()
        print(radar_data.max())
        ...
        for batch_idx in range(B):
            arr_cube = radar_data[batch_idx]
            z_ind, y_ind, x_ind = torch.where(arr_cube > arr_cube.quantile(quantile_rate))

            power_val = arr_cube[z_ind, y_ind, x_ind].unsqueeze(-1)
            
            z_pc_coord = ((z_min + z_ind * grid_size) - grid_size / 2).unsqueeze(-1).cuda()
            y_pc_coord = ((y_min + y_ind * grid_size) - grid_size / 2).unsqueeze(-1).cuda()
            x_pc_coord = ((x_min + x_ind * grid_size) - grid_size / 2).unsqueeze(-1).cuda()
            sparse_rdr_cube = torch.cat((x_pc_coord, y_pc_coord, z_pc_coord, power_val), dim=-1).to(dtype).cuda()
            ...
            z_ind = torch.ceil((z_pc_coord.squeeze(-1) - z_min) / grid_size).long()
            y_ind = torch.ceil((y_pc_coord.squeeze(-1) - y_min) / grid_size).long()
            x_ind = torch.ceil((x_pc_coord.squeeze(-1) - x_min) / grid_size).long()
            sp_indices = torch.cat((batch_indices, z_ind.unsqueeze(-1), y_ind.unsqueeze(-1), x_ind.unsqueeze(-1)), dim=-1)
            list_sp_indices.append(sp_indices)
        
        # Concatenate all batch-wise processed sparse_rdr_cubes and sp_indices
        ...
        # Now you can use `sparse_rdr_cube_all_batches` and `sp_indices_all_batches` for further processing.
        # Ensure that the downstream methods can handle the batch dimension correctly.
        batch_size = B
        ...
        torch.cuda.synchronize()
        print("attention: ",start.elapsed_time(end))
