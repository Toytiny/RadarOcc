...
class OccHead(nn.Module):
    def __init__(
        ...
        if balance_cls_weight:
            self.class_weights = torch.from_numpy(1 / np.log(kradar_easy_class_frequencies + 0.001))
        else:
            self.class_weights = torch.ones(class_num)/class_num  # FIXME hardcode 
            print("NO using weight in loss")
    ...
    def loss_voxel(self, output_voxels, target_voxels,tag,sigmoid_cube=None):
        loss_dict = {}
        # if sigmoid_cube is not None:
        #     ###supervise voxel selection
        #     loss_dict['loss_voxel_select_{}'.format(tag)] =  self.loss_voxel_geo_scal_weight * geo_scal_loss(output_voxels, target_voxels, ignore_index=255, non_empty_idx=self.empty_idx)
        # # resize gt                       
        ...
        # output_voxels = output_voxels.to(torch.float32) #only for 
    ...
    def loss(self, output_voxels=None,
                ...
                target_voxels=None, visible_mask=None, sigmoid_cube=None, **kwargs):
        loss_dict = {}
        for index, output_voxel in enumerate(output_voxels):
            loss_dict.update(self.loss_voxel(output_voxel, target_voxels, tag='c_{}'.format(index),sigmoid_cube=sigmoid_cube))
        if self.cascade_ratio != 1:
            loss_batch_dict = {}
            if self.sample_from_voxel or self.sample_from_img:
