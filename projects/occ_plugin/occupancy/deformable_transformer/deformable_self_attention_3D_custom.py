...
class DeformSelfAttention3DCustom(BaseModule):
    ...
    def __init__(self,
        ...
        if fp16_enabled:
            self.sampling_offsets = nn.Linear(
                embed_dims*self.num_bev_queue, num_bev_queue*num_heads * num_levels * num_points * 3).half()
            self.attention_weights = nn.Linear(embed_dims*self.num_bev_queue,
                                            num_bev_queue*num_heads * num_levels * num_points).half()
            self.value_proj = nn.Linear(embed_dims, embed_dims).half()
            self.output_proj = nn.Linear(embed_dims, embed_dims).half()
        else:
            self.sampling_offsets = nn.Linear(
    ...
    def forward(self,
        ...
        if value is None:
            ...
            value = torch.stack([value, value], 1).reshape(bs*2, len_bev, c)
