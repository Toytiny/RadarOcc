...
class VoxFormerEncoder3D(TransformerLayerSequence):
    ...
    def point_sampling(self, reference_points, pc_range):
        ...
        reference_points = reference_points.view(
            D, B, 1, num_query, 4).repeat(1, 1, 1, 1, 1).unsqueeze(-1)
    ...
    def forward(self,
        ...
        h,w,z = spatial_shapes

        ref_2d = self.get_reference_points(
            h, w, z, dim='3dCustom', bs=bev_query.size(1), device=bev_query.device, dtype=bev_query.dtype)
        ...
        if sph_ref_3d is not None: #spherical to cart attention:
           sph_ref_3d = sph_ref_3d.unsqueeze(0).repeat(bs, 1, 1) #b,N,3
           sph_ref_3d = sph_ref_3d.unsqueeze(2) #b,N,1(layer),3
           ref_2d = ref_3d = sph_ref_3d
