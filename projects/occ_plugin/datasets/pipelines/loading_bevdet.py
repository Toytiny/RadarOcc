    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            if self.load_depth:
                ...
                seq_name = results['curr']['cams'][cam_name]['data_path'].split('/')[-3]
                depth_path = os.path.join(self.depth_gt_path, seq_name+file_name+'.bin')
                point_depth = np.fromfile(depth_path,
                    dtype=np.float32,
                    count=-1).reshape(-1, 3)
