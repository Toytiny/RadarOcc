...
class NuscOCCDataset(NuScenesDataset):
    ...
    def evaluate(self, results, logger=None, **kawrgs):
        ...
        res_table, res_dic = format_SC_results(ious[1:], return_dic=True)
        for key, val in res_dic.items():
            eval_results['SC1_{}'.format(key)] = val
        if logger is not None:
            logger.info('SC1 Evaluation')
            logger.info(res_table)
        ...
        res_table, res_dic = format_SC_results(ious[1:], return_dic=True)
        for key, val in res_dic.items():
            eval_results['SC2_{}'.format(key)] = val
        if logger is not None:
            logger.info('SC2 Evaluation')
            logger.info(res_table)
        ...
        res_table, res_dic = format_SSC_results(ious, return_dic=True)
        for key, val in res_dic.items():
            eval_results['SSC1_{}'.format(key)] = val
        if logger is not None:
            logger.info('SSC1 Evaluation')
            logger.info(res_table)
        ...
        res_table, res_dic = format_SSC_results(ious, return_dic=True)
        for key, val in res_dic.items():
            eval_results['SSC2_{}'.format(key)] = val
        if logger is not None:
            logger.info('SSC2 Evaluation')
            logger.info(res_table)        
        ''' evaluate SSC_fine '''
