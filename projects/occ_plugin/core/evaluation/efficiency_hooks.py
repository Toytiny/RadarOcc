...
class OccEfficiencyHook(Hook):
    ...
    def construct_input(self, DUMMY_SHAPE=None, m_info=None):
        ...
        if 'points' in m_info.keys():
            ...
            input['points'] = points
        if 'rdr_cube' in m_info.keys():
            rdr_cube = m_info['rdr_cube']
            rdr_cube = rdr_cube.cuda()
            input['rdr_cube'] = rdr_cube
        if 'rdr_tensor' in m_info.keys():
            rdr_tensor = m_info['rdr_tensor']
            rdr_tensor = rdr_tensor.cuda()
            input['rdr_tensor'] = rdr_tensor
        if 'sparse_radar' in m_info.keys():
            data = m_info['sparse_radar']
            data['power_val'] = data['power_val'].cuda()
            data['range_ind'] = data['range_ind'].cuda()
            data['elevation_ind'] = data['elevation_ind'].cuda()
            data['azimuth_ind'] = data['azimuth_ind'].cuda()
            input['sparse_radar'] = data
        return input
